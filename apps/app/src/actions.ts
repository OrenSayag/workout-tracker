...
export async function getUserUnreadAchievementsAction() {
  const session = await auth();
  if (!session?.user?.id) {
    throw new Error('Not authenticated');
  }

  return getUserUnreadAchievement({ userId: session.user.id });

}

export async function markAchievementReadAction(userAchievementIdToMark: string) {
  const session = await auth();
  if (!session?.user?.id) {
    throw new Error('Not authenticated');
  }
  return markAchievementRead({ userAchievementId:userAchievementIdToMark });
