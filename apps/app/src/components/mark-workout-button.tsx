...
import { useMarkWorkout, useTodaysWorkoutStatus } from '../hooks/use-workouts';
  ...
  if ((status as { completed: boolean } | undefined)?.completed) {
    return (
      <div className="flex flex-col items-center space-y-2">
        <span role="img" aria-label="celebration" style={{ fontSize: 48 }}>
          🎉
        </span>
  ...
  return (
    <div className="space-y-2">
      <Button
        ...
        className="w-full"
      >
        {markWorkoutMutation.isPending ? 'Marking...' : "Mark Today's Workout"}
      </Button>
      {message && (
        <p
          className={`text-sm ${
